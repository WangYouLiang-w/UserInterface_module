...
public class Config_TestOffline : MonoBehaviour
{
        ...
        else
        {
            if (Offline_flag == true)//离线实验
            {
                switch (Mode_Offline)
                {
                    case 1://刺激态                   
                        ...
                        {
                            if (time_sti == 0) { Neuroscan_ParallelPortLabel(Random_num[qune] + 1); }
            ...
            else if (StiOnline_flag == true)//模拟在线实验
            {
                ...
                if (Label_number < (stionline_sti_fragment * Sti_number * stionline_sti_round))
                {
                    ...
                    else
                    {
                        ...
                        if (time_sti == 0) { Neuroscan_ParallelPortLabel(Random_num[qune] + 1); Label_number++; }
                        if (time_sti < time_sti_lengeth)
                        {
                            for (int j = 0; j < Sti_number; j++)
                            {
            ...
            else if (Online_flag == true)//在线实验,Result为回传的分类结果
            {
                ...
                if (time_sti == 0)
                {
                    ...
                    Neuroscan_ParallelPortLabel(Stionline_lable);
                    Debug.Log("Stionline_lable" + Stionline_lable);
                }
                if (time_sti < time_sti_lengeth)
                {
    ...
    private void Pre_settings(string PreJsonStr)
    {
        ...
        if (psr.ReadToEnd() != string.Empty)
        {
            ...
            time_sti_lengeth = float.Parse(DataJson[1][0].ToString());
            offline_time_cue = float.Parse(DataJson[2][0].ToString());
            offline_sti_fragment = int.Parse(DataJson[3][0].ToString());
            offline_sti_round = int.Parse(DataJson[4][0].ToString());
            stionline_sti_fragment = int.Parse(DataJson[5][0].ToString());
            stionline_sti_round = int.Parse(DataJson[6][0].ToString());
    ...
    void Neuroscan_ParallelPortLabel(int label)
    {
        ...
        StartCoroutine(WaitForSeconds(0.005f, () => { PortAccess.paralleOutput(portAddress, label); }));
