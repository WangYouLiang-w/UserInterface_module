...
public class New_module : MonoBehaviour
{
    ...
    public float[] Sti_frequency = { 31.00f, 32.00f, 33.00f, 34.00f, 35.00f,
    ...
    //public float[] Sti_phase = { 0f, 1.65f, 1.30f, 0.95f, 0.60f, 0.25f, 1.90f, 1.55f, 1.20f, 0.85f, 0.50f, 0.15f, 1.80f, 1.45f, 1.10f, 0.75f, 0.50f, 0.25f };//EXE_0716 第一次
     public float[] Sti_phase = { 0.00f, 1.40f, 0.80f, 0.20f, 1.60f,
            ...
            else if (Online_flag == true)//在线实验,Result为回传的分类结果
            {
                ...
                if (time_sti < 0.5)
                {
                    for (int j = 0; j < Sti_number; j++)
                    {
                        ...
                        {
                            gray_value = (float)((Math.Sin(2.0f * Math.PI * Sti_Highfre[j] * time_sti + Math.PI * Sti_Highphase[j]) + 1f) / 2f);
                            //gray_value = (float)Convert.ToDouble(((Math.Sin(2.0f * Math.PI * Double.Parse(Sti_Highfre[j]) * time_sti + Math.PI * Double.Parse(Sti_Highphase[j])) + 1f)) / 2f);
                            //gray_value = (float)Convert.ToDouble(((Math.Sin(2.0f * Math.PI * Double.Parse(Sti_Highfre[j]) * time_sti) + 1f)) / 2f);
                            Sti_command[j].GetComponent<Image>().color = new Color(gray_value, gray_value, gray_value, 255f);
